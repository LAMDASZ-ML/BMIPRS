...
class AID(DatasetBase):
    ...
    dataset_dir = "AID"  # 数据集在文件系统中的根目录名称

    def __init__(self, cfg):
        # 解析数据集根目录（从配置中获取并标准化路径）
        root = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))
        self.dataset_dir = os.path.join(root, self.dataset_dir)  # 完整数据集路径
        self.image_dir = os.path.join(self.dataset_dir, "AID_dataset")  # 图像存放目录
        self.split_path = os.path.join(self.dataset_dir, "split_chen_aid.json")  # 数据分割文件路径
        ...
        if os.path.exists(self.split_path):
            ...
            print(f"Generating new data split for AID and saving to {self.split_path}")
