...
def metrics_base(predicts, targets, classnames, base_classnames):
    ...
    base_total= np.sum(sample_selector)
    base_predicts = predicts.copy()

    base_correct = np.sum(np.argmax(base_predicts[sample_selector, :], axis=1) == targets[sample_selector])
    total = len(predicts)
    return base_correct, base_total, total
...
def metrics_new(predicts, targets, classnames, base_classnames, all_classnames):
    ...
    sample_selector = [ classnames[i] in base_classnames     for i in range(len(classnames)) ]
    class_selector  = [ all_classnames[i] in base_classnames for i in range(len(all_classnames)) ]
    gts = [ classnames[i] in base_classnames for i in range(len(classnames)) ]
    # print(gts)
    gts = np.array(gts)
    # print(gts.shape)
    ...
    base_correct = np.sum(np.argmax(base_predicts[sample_selector, :], axis=1) == targets[sample_selector])
    new_correct  = np.sum(np.argmax(new_predicts[~sample_selector, :], axis=1) == targets[~sample_selector])
    probs = F.softmax(torch.tensor(predicts), dim=1).cpu()
    probs = torch.max(probs.view(probs.shape[0], -1), 1)[0].cpu().numpy()
    auroc = -1
    if len(set(gts)) > 1:
    ...
    return base_correct, base_total, new_correct, new_total, correct, total, auroc
