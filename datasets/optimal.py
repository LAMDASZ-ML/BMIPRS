...
# OPTIMAL数据集的类名映射表（将数据集原始类名映射为规范名称）
NEW_CNAMES = {
...
class OPTIMAL(DatasetBase):
    """OPTIMAL遥感图像数据集类，继承自Dassl的DatasetBase基类"""
    
    dataset_dir = "OPTIMAL"  # 数据集在文件系统中的根目录名称

    def __init__(self, cfg):
        """
        初始化OPTIMAL数据集，加载数据并处理分割
        
        Args:
            cfg: 配置对象，包含数据集路径、少样本参数等信息
        """
        ...
        self.split_path = os.path.join(self.dataset_dir, "split_chen_optimal.json")  # 数据分割文件路径
        ...
        if os.path.exists(self.split_path):
            ...
            print(f"Generating new data split for OPTIMAL and saving to {self.split_path}")
