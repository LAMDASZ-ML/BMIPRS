...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    ...
    for subdir in subdirs:
        ...
        # 如果subdir名称中有log
        if "log" in subdir:
            fpath = osp.join(directory, subdir)
        else:
    ...
    for output in outputs:
        ...
        if not args.multi_exp:
            ...
            print(msg)
        accuracy_results.append(output['accuracy'])
    # 将每个accuracy_results中空隙使用\t连接
    accuracy_results = "\t".join([str(i) for i in accuracy_results])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ratio", action="store_true", help="parse ratio")
    parser.add_argument(
