...
def load_clip_to_cpu(cfg):
    ...
    if cfg.TRAINER.BMIPMIX.CONNECT_METHOD == "joint":
        BMIP_length = cfg.TRAINER.BMIPMIX.N_CTX * 2
        connect_method = "joint"
    else:
        BMIP_length = cfg.TRAINER.BMIPMIX.N_CTX
        connect_method = "other"
    # print(f"weight_threshold: {cfg.TRAINER.BMIPMIX.WEIGHT_THRESHOLD}")
    # weight_threshold = cfg.TRAINER.BMIPMIX.WEIGHT_THRESHOLD
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.BMIPMIX.N_CTX
        ctx_init = cfg.TRAINER.BMIPMIX.CTX_INIT
        ...
        assert cfg.TRAINER.BMIPMIX.PROMPT_DEPTH >= 1, "For BMIP, PROMPT_DEPTH should be >= 1"
        self.compound_prompts_depth = cfg.TRAINER.BMIPMIX.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
        assert cfg_imsize == clip_imsize, f"cfg_imsize ({cfg_imsize}) must equal to clip_imsize ({clip_imsize})"
        # if cfg.TRAINER.BMIPMIX.CONNECT_METHOD == "joint":
        #     tmp_ctx = cfg.TRAINER.BMIPMIX.N_CTX * 2
        # else:
        #     tmp_ctx = cfg.TRAINER.BMIPMIX.N_CTX
        if cfg.TRAINER.BMIPMIX.CONNECT_METHOD != "joint":
        ...
        # self.weight_threshold = cfg.TRAINER.BMIPMIX.WEIGHT_THRESHOLD
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.connect_method = cfg.TRAINER.BMIPMIX.CONNECT_METHOD
    ...
    def forward(self, image, label=None):
        ...
        B, N, d = patch_features.shape
        h = w = int(N ** 0.5)  # 假设 14x14
        y, x = torch.meshgrid(torch.arange(h), torch.arange(w), indexing='ij')
        center = h // 2
        dist = (y - center) ** 2 + (x - center) ** 2
        dist = dist.float().to(patch_features.device)
        confidence = 1.0 - dist / dist.max()  # 中心=1，边缘=0
        confidence = confidence.view(1, N, 1)  # [1, N, 1]
        confidence = confidence.to(self.dtype)
        patch_features = patch_features * confidence
...
class BMIPMIX(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.BMIPMIX.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.BMIPMIX.PREC == "fp32" or cfg.TRAINER.BMIPMIX.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.BMIPMIX.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.BMIPMIX.PREC
        if prec == "amp":
            with autocast():
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            logits_1, logits_2, logits_3 = self.model_inference(input)
            # # 🔹 分别取每个的预测类别
