...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # ====== 新增：加载原始 CLIP 用于生成 zero-shot logits ======
        with torch.no_grad():
            orig_clip = load_original_clip(cfg)
            # 关键：强制转为 float32（避免 LayerNorm mixed dtype）
            orig_clip = orig_clip.float()
            # 必须移到 GPU！避免在 CPU 上运行（CPU 不允许 mixed dtype）
            orig_clip = orig_clip.cuda()  # 或 .to(self.logit_scale.device)

            classnames_clean = [name.replace("_", " ") for name in classnames]
            prompts_zs = [f"a photo of a {name}." for name in classnames_clean]
            tokenized_zs = torch.cat([clip.tokenize(p) for p in prompts_zs]).cuda()

            text_feats_zs = orig_clip.encode_text(tokenized_zs)
            text_feats_zs = text_feats_zs / text_feats_zs.norm(dim=-1, keepdim=True)
            self.register_buffer("zero_shot_text_features", text_feats_zs)

            self.zero_shot_image_encoder = orig_clip.visual
            self.zero_shot_image_encoder.eval()
            for p in self.zero_shot_image_encoder.parameters():
    ...
    def forward(self, image, label=None):
        ...
        # Compute zero-shot logits using frozen original CLIP
        with torch.no_grad():
            # 强制输入为 float32，因为 zero_shot_image_encoder 是 float32 模型
            img_feat_zs = self.zero_shot_image_encoder(image.float())
            img_feat_zs = img_feat_zs / img_feat_zs.norm(dim=-1, keepdim=True)
            zero_shot_logits = logit_scale * img_feat_zs @ self.zero_shot_text_features.t()
            zero_shot_logits = zero_shot_logits.type_as(logits_trainable)

        ce_loss = F.cross_entropy(logits_trainable, label)
        return ce_loss, logits_trainable, zero_shot_logits
...
class BMIPKL(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                ce_loss, logits_trainable, zero_shot_logits = model(image, label)
                # KL loss with log_target=True
                log_prob_train = F.log_softmax(logits_trainable / T, dim=1)
                log_prob_zs = F.log_softmax(zero_shot_logits / T, dim=1)
                kl_loss = F.kl_div(log_prob_train, log_prob_zs, reduction='sum', log_target=True)
                kl_loss = kl_loss * (T * T) / logits_trainable.numel()
