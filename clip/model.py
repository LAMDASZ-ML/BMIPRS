...
class ResidualAttentionBlock_BMIP(nn.Module):
    ...
    def forward(self, inputs, no_text_exchange = False, no_visual_exchange = False):
        # For the first layer, we do not need to add any duplicate, as it is already added
        # as the shallow version
        x = inputs[0]
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper_local) - 1):
                        ...
                        if weight is None:
                            ...
                            if no_visual_exchange:
                                visual_context = compound_prompts_deeper_local[counter].expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                            else:
                                visual_prompt = []
                                tmp_x, current_weight = self.attention(self.ln_1(x))
                                current_weight = current_weight.mean(dim=(0))                       
                                current_weight = current_weight[0:x.shape[0] - self.compound_prompt_nctx, :] 
                    ...
                    if not (counter > len(compound_prompts_deeper_local) - 2):
                        ...
                        if weight is None:
                            ...
                            if no_text_exchange:
                                textual_context = compound_prompts_deeper_local[counter].expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                            else:
                                textual_prompt = []
                                tmp_x, current_weight = self.attention(self.ln_1(x))
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, no_text=False, no_visual=False):
        if isinstance(self.resblocks, nn.Sequential):
            return self.resblocks(x)
        else:
            for block in self.resblocks:
                x = block(x, no_text, no_visual)
            return x
...
class VisionTransformer_BITP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, no_text = False, no_visual=False, return_patches=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        # Again combine the inputs, so nn.sequential can work
        outputs = self.transformer([x, compound_deeper_prompts, 0], no_text, no_visual)  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD

        ######################################
        if return_patches:
            # 返回所有 patch 特征 (不含 CLS)
            patch_tokens = x[:, 1:1 + (self.input_resolution // self.conv1.kernel_size[0])**2, :]
